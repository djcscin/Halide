...
namespace {
    ...
    public:
        ...
        void generate() {
            ...
            diff_x(x, y, i, c) = absd(guide_bound(x, y, c), guide_bound(x + i, y, c));
            norm_x(x, y, i) = u16_sat((diff_x(x, y, i, 0) + diff_x(x, y, i, 1) + diff_x(x, y, i, 2))*max14_f32);
            weights_x(x, y, i) = weights_spatial(i) * weights_range(norm_x(x, y, i));

            // sum_x(x, y, c) = 0.f;
            sum_x(x, y, c) += select(c == channel_min-1, weights_x(x, y, kernel),
                                weights_x(x, y, kernel) * output_y(x + kernel, y, c));

            output_x(x, y, c) = sum_x(x, y, c)/sum_x(x, y, channel_min-1);

            output(x, y, c) = clamp(output_x(x, y, c), 0.f, 1.f);
        }
